const { OutletService, OutletServiceCategory } = require('../models');
...
exports.list = async (req, res) => {
  try {
    ...
    const rows = await OutletService.findAll({
      where,
      include: [{ model: OutletServiceCategory, as: 'serviceCategory', attributes: ['id', 'name', 'key'], where: { status: 'active' }, required: true }],
      order: [
        [{ model: OutletServiceCategory, as: 'serviceCategory' }, 'sortOrder', 'ASC'],
        ['sortOrder', 'ASC'],
        ['id', 'ASC'],
      ],
    });
    ...
    console.error('[OutletService] list error:', err.message);
...
exports.detail = async (req, res) => {
  try {
    ...
    const service = await OutletService.findByPk(id, {
      include: [{ model: OutletServiceCategory, as: 'serviceCategory', attributes: ['id', 'name', 'key'] }],
    });
    if (!service) return res.status(404).json({ code: 404, message: '服务不存在' });
    res.json({ code: 0, data: service });
  } catch (err) {
    console.error('[OutletService] detail error:', err.message);
...
exports.adminList = async (req, res) => {
  try {
    ...
    console.error('[OutletService] adminList error:', err.message);
    res.status(500).json({ code: 500, message: '获取服务列表失败' });
  }
};
...
exports.create = async (req, res) => {
  try {
    ...
    const service = await OutletService.create(data);
    const withCat = await OutletService.findByPk(service.id, { include: [{ model: OutletServiceCategory, as: 'serviceCategory' }] });
    res.json({ code: 0, data: withCat || service });
  } catch (err) {
    console.error('[OutletService] create error:', err.message);
...
exports.update = async (req, res) => {
  try {
    ...
    console.error('[OutletService] update error:', err.message);
...
exports.remove = async (req, res) => {
  try {
    ...
    console.error('[OutletService] remove error:', err.message);
